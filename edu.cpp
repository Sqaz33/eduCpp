...
int main() {
    ...
    static constinit int c = foo(1, 2); // гарантирует инициализацию на этапи конпиляции
}
...
struct is_same_types :
    std::conjunction<std::is_same<A, B>, is_same_types<B, T...>>
{};

template <class A, class B>
struct is_same_types<A, B> :
    std::is_same<A, B>
{};

template <class A>
struct is_same_types<A, A> :
    std::true_type
{};

template <class ... T>
static constexpr bool is_same_types_v = is_same_types<T...>::value;
...
template <class... T>
    requires is_same_types_v<T...> 
int max(const T& ... t) {
    return ((std::max(INT_MIN, t)), ...);
}

} // namespace

int main() {
    std::cout << is_same_types_v<char, int> << '\n';
    print(1, .1, 's', "1234");
    std::cout << max(1, 2, 3, 3) << '\n';
}
